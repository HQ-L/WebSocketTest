        ...
        for dataCity in data["city"]:
            for item in params["result"][2]:
                if item["id"][-2:] != '00' and dataCity["id"][0:4] == item["id"][0:4]:
                    dataCity["county"].append({"id": item["id"], "name": item["fullname"], "longitude": str(item["location"]["lng"]), "dimension": str(item["location"]["lat"])})
...
with open('outputData.json', 'w', encoding='utf-8') as jsonFile:
