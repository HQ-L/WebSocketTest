...
class UIViewTools {
    ...
    static var viewString: [String] = []

    static func printViewHierachy() -> [String] {
        guard let view = UIApplication.shared.keyWindow else {
            return viewString
        }

        return printViewHierachy(for: view, level: 0)
...
private extension UIViewTools {
    ...
    static func printViewHierachy(for view: UIView, level: Int) -> [String] {
        viewString.append(printDetail(for: view, level: level))
    ...
    static func printDetail(for view: UIView, level: Int) -> String {
        ...
        return "\(indent)\(accessibilityLabel)(\(detail(for: view)))"
    }
